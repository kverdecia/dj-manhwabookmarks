...
def coverage(c):
    ...
    """
    c.run("coverage run --source dj-manhwabookmarks runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/dj-manhwabookmarks.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ djmanhwabookmarks")
...
def lint(c):
    ...
    """
    c.run("flake8 dj-manhwabookmarks tests")
...
def release(c, bumpsize=''):
    ...
    import dj_manhwabookmarks
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=dj_manhwabookmarks.__version__))
    c.run("git push --tags")
